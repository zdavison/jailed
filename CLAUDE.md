...
- **`install.sh` embeds byte-identical copies** of `bin/jailed`, `hooks/jailed-hook.sh`, `config/commands.default`, `config/srt-settings.json`, and `config/apparmor-bwrap.profile` as heredoc strings (`JAILED_SCRIPT`, `JAILED_HOOK_SCRIPT`, `DEFAULT_COMMANDS`, `SRT_SETTINGS`, `APPARMOR_BWRAP_PROFILE`). Makes `curl | bash` work with no other files. `tests/test_installer.sh` fails if any embedded copy diverges from its source — **edit both when changing either.**
- **AppArmor userns preflight (Linux).** `ensure_bwrap_userns` runs before `install_bins`. If `/proc/sys/kernel/apparmor_restrict_unprivileged_userns == 1` (Ubuntu 24.04+ default), it writes a narrow AppArmor profile to `/etc/apparmor.d/bwrap` that grants `userns` to the resolved `bwrap` path only, then `apparmor_parser -r`'s it. Without this, SRT's deny-network mode fails with `bwrap: loopback: Failed RTM_NEWADDR` because AppArmor blocks bringing up `lo` in the unshared net namespace. A marker line (`# jailed: apparmor-bwrap-profile`) distinguishes our profile from distro/user-packaged ones — if an unmarked profile exists at that path, we leave it alone; on uninstall, `remove_apparmor_profile` only removes profiles carrying the marker. Env overrides `JAILED_APPARMOR_USERNS_FILE`, `JAILED_APPARMOR_PROFILE_PATH`, `JAILED_APPARMOR_NO_RELOAD` exist for tests so the logic runs without root.
